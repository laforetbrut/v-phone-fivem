...
- **A real licence.** The project shipped under something called "MIT License with Attribution
  Requirement", which is not the MIT licence: adding a condition to MIT makes it a bespoke
  licence, and a bespoke licence is what makes a scanner flag a resource and a cautious server
  owner walk away. It is now the **Apache License 2.0**, unmodified, with an SPDX identifier and
  a patent grant, and the attribution lives in a `NOTICE` file where section 4(d) of that licence
  puts it. Section 4(d) obliges a derivative work to carry that file and to show its contents
  wherever the work normally displays the notices of what it is built on. For this phone that
  place is Settings > About, where the author is already shown.

  What changes for you: nothing, if you run it as it ships. If you redistribute a modified copy,
  keep `NOTICE` and keep the credit reachable. What changed against the old terms: the old wording
  said removing the credit terminated your rights outright, which was stronger. In exchange the
  licence is now one people recognise.

- **The interface has a name of its own: FruitOS.** Every part of the phone that borrowed a name
  from a real product now uses its own. The custom-property prefix is `--fruitos-`, sharing is
  **FruitDrop**, unlocking with your face is **Face Unlock**, the panel of switches is **Quick
  Settings**, the pill at the top is **the Island**, searching from the home screen is **Search**,
  message reactions are **reactions**, and the app shop was always **FruitStore**. 925 occurrences
  across 24 files, including 36 locale keys, several CSS classes, four network events and two NUI
  callbacks.

  **If you keep an edited `config.lua` across this update:** `Config.Airdrop` is now
  `Config.FruitDrop`. Any range or lifetime you tuned there falls back to the default until you
  rename the table. Nothing else in the config moved.

  A saved preference is migrated for you: `prefs.faceId` is read once as `faceUnlock` and rewritten
  on the next save, so nobody has to set up Face Unlock again. The inventory item `iphone` is
  **deliberately still accepted**, because it sits in existing servers' item catalogues and in
  players' inventories, and dropping it would lock those players out of their phone with nothing
  on screen to explain why. It is matched, never displayed.

- **The Bleeter icon is drawn rather than borrowed.** It was a bird silhouette in a brand blue
  that belong to somebody else. It is now a hash mark in this resource's own house style, which is what the app is actually about: it has hashtags, a tag feed and a trending list, and the
  app's accent is a colour of its own. Bleeter itself is Rockstar's fiction and stays.

### Notes

- No third-party asset is included in this repository, and `NOTICE` now says so and shows the
  working: the tones are generated by a script that ships with the source, the icons are inline
  paths, the wallpapers are CSS gradients, and no font file is distributed.

- `CHANGELOG.md` and `ERROR_LOG.md` still contain the old vocabulary in the entries for releases
  that already shipped. That is deliberate. They are the record of what was released and when, and
  editing what a past version said would make them worth less than nothing.

---

### Changements (miroir francais)

- **Une vraie licence.** Le projet etait sous une licence intitulee "MIT License with Attribution
  Requirement", qui n'est pas la licence MIT : ajouter une condition a MIT en fait une licence
  maison, et c'est une licence maison qui fait qu'un outil d'analyse signale une ressource et
  qu'un proprietaire de serveur prudent passe son chemin. C'est desormais la **licence Apache
  2.0**, non modifiee, avec un identifiant SPDX et une clause de brevet, et l'attribution vit dans
  un fichier `NOTICE`, la ou l'article 4(d) de cette licence la place. Cet article oblige un
  travail derive a transporter ce fichier et a en afficher le contenu partout ou ce travail montre
  normalement les mentions des logiciels dont il depend. Pour ce telephone, cet endroit est
  Reglages > A propos, ou l'auteur est deja affiche.

  Ce qui change pour vous : rien, si vous l'utilisez tel quel. Si vous redistribuez une copie
  modifiee, gardez `NOTICE` et gardez le credit accessible. Ce qui change par rapport aux anciens
  termes : l'ancienne formulation disait que retirer le credit mettait fin a vos droits, ce qui
  etait plus fort. En echange, la licence est maintenant une licence que les gens reconnaissent.

- **L'interface porte son propre nom : FruitOS.** Chaque partie du telephone qui empruntait son nom
  a un vrai produit utilise desormais le sien. Le prefixe des proprietes est `--fruitos-`, le
  partage est **FruitDrop**, le deverrouillage par le visage est **Face Unlock**, le panneau
  d'interrupteurs est **Quick Settings**, la pilule en haut de l'ecran est **l'ile**, la recherche
  depuis l'accueil est **Search**, les reactions aux messages sont des **reactions**, et la
  boutique s'appelait deja **FruitStore**. 925 occurrences dans 24 fichiers, dont 36 cles de
  locale, plusieurs classes CSS, quatre evenements reseau et deux callbacks NUI.

  **Si vous conservez un `config.lua` modifie a travers cette mise a jour :** `Config.Airdrop`
  s'appelle maintenant `Config.FruitDrop`. La portee et la duree de vie que vous y aviez reglees
  retombent sur les valeurs par defaut tant que la table n'est pas renommee. Rien d'autre n'a
  bouge dans la configuration.

  Une preference enregistree est migree pour vous : `prefs.faceId` est relue une fois comme
  `faceUnlock` puis reecrite a la sauvegarde suivante, donc personne n'a a reconfigurer Face
  Unlock. L'objet d'inventaire `iphone` reste **volontairement accepte**, parce qu'il figure dans
  le catalogue d'objets des serveurs existants et dans l'inventaire des joueurs : le retirer
  enfermerait ces joueurs dehors sans rien afficher pour l'expliquer. Il est reconnu, jamais
  affiche.

- **L'icone de Bleeter est dessinee, plus empruntee.** C'etait une silhouette d'oiseau dans un
  bleu de marque appartenant a quelqu'un d'autre. C'est maintenant un diese dans le style
  maison de la ressource, et l'accent de l'application a sa propre couleur. Bleeter lui-meme est
  de la fiction Rockstar et reste.

### Notes (miroir francais)

- Aucun element appartenant a un tiers n'est present dans ce depot, et `NOTICE` le dit desormais
  en montrant le raisonnement : les tonalites sont generees par un script livre avec la source,
  les icones sont des chemins en ligne, les fonds d'ecran sont des degrades CSS, et aucun fichier
  de police n'est distribue.

- `CHANGELOG.md` et `ERROR_LOG.md` contiennent toujours l'ancien vocabulaire dans les entrees des
  versions deja sorties. C'est volontaire. Ce sont les registres de ce qui a ete publie et quand,
  et modifier ce qu'une version passee disait les rendrait moins que sans valeur.
...
- **Half the emoji keyboard could not be reached.** 1.5.5 grew the picker from four categories to ten and let the tab strip scroll sideways when they did not fit. That reads fine on a touchscreen and is unusable here: this is a NUI page and the player has a MOUSE CURSOR, so a horizontal scroller has no gesture behind it at all - the last categories were simply off the right edge with nothing able to bring them back. The tabs shrink to fit now, all ten of them, on the narrowest phone. **This was verified the wrong way the first time**: "the strip scrolls" was measured and called a success, when the question that mattered was whether a player could click the tenth tab. It is measured per tab now, and each one is confirmed clickable where the cursor would actually land.
- **The same fault, in six more places.** `.seg.scroll` is used for the store's categories, the gallery's albums, the camera's filters and crops, Zuber's tiers and the social trends, and every one of them had the same dead edge - the store's "Work", "On duty", "Entertainment", "Health" and "Essentials" could not be selected. One of them had already been noticed and worked around locally, which is what a bug looks like when it is fixed one instance at a time. Every sideways strip in the phone now answers the two gestures a cursor actually has: the wheel, turned sideways when the strip has no vertical scrolling of its own, and a press-and-drag with a threshold so a tap is still a tap. Added once, so a strip written next month gets it for free.
- **The home indicator's drag zone reached into the emoji tabs.** The panel arms its swipe-to-dismiss on anything pressed in its top sixty pixels, and the tabs were measured at sixty-one - one pixel of clearance, on one phone. It stops where the controls start now, computed rather than guessed.
- **"Developed by vyrriox" was printed twice** in Settings, once as a row and once as a sentence under it. The row stays, which is the attribution the licence asks for; the sentence is gone.

### Added

- **`phone:Call(source, number)` and `phone:EndCall(source)`.** A script can place and end a call. It goes THROUGH the phone's own dialler rather than around it, so every rule a player meets applies: the caller needs a phone and a signal, the other end must be connected and not already on a call, Do Not Disturb is honoured, and anybody standing near the ringing phone hears it. `{ anonymous = true }` withholds the number, `{ video = true }` makes it a video call. Ending a conference removes that one person rather than ending it for everybody.
- **`phone:GetMessages(citizenid, number, limit)`** - what was said in a conversation, oldest first. Reading does **not** mark the thread read: a script asking what was said has not read it on the player's behalf, and quietly clearing somebody's unread badge would be a side effect nobody asked for and nobody could see. Messages that character deleted from their own copy stay hidden, so this is not a way to read past the delete button.
- **A savings account that cannot be read now says why.** Three situations look identical from the app - no savings account, a table that is not there, and a read that failed - and only one of them is normal. The console names which, once, the first time it happens.
- **`tools/probe.js`** - the question no static check can ask: with the phone drawn, could a cursor actually click this? Off the screen, zero-sized, covered by something else, or only arrivable by scrolling a container sideways all count as no. It installs all thirty-three apps first, because the preview boots like a fresh install and twelve are gated - twenty-one of thirty-three is not a sweep. This is the check that would have caught the emoji tabs, and it found the store's categories the first time it ran.

### Ajouts et correctifs (miroir français)

- **La moitié du clavier émoji était inatteignable.** La 1.5.5 a fait passer le sélecteur de quatre catégories à dix et laissait la bande d'onglets défiler latéralement quand elles ne tenaient pas. C'est correct sur un écran tactile et inutilisable ici : c'est une page NUI et le joueur a une SOURIS, donc un défilement horizontal n'a aucun geste derrière lui - les dernières catégories étaient simplement hors du bord droit sans rien pour les ramener. Les onglets se compriment désormais pour tenir, tous les dix, sur le téléphone le plus étroit. **La vérification était fausse la première fois** : j'ai mesuré « la bande défile » et appelé ça une réussite, alors que la question qui comptait était de savoir si un joueur pouvait cliquer le dixième onglet. C'est mesuré onglet par onglet maintenant, et chacun est confirmé cliquable là où le curseur atterrit vraiment.
- **Le même défaut, à six autres endroits.** `.seg.scroll` sert aux catégories du magasin, aux albums de la galerie, aux filtres et cadrages de l'appareil photo, aux paliers Zuber et aux tendances sociales : tous avaient le même bord mort, et « Travail », « En service », « Divertissement », « Santé » et « Essentiels » du magasin étaient inélectionnables. L'un d'eux avait déjà été remarqué et contourné localement, ce à quoi ressemble un bug corrigé un cas à la fois. Toute bande horizontale du téléphone répond maintenant aux deux gestes qu'un curseur possède réellement : la molette, tournée sur le côté quand la bande n'a pas de défilement vertical propre, et un appui-glisser avec un seuil pour qu'une pression reste une pression. Ajouté une fois, donc une bande écrite le mois prochain en hérite.
- **La zone de glissement de la barre d'accueil mordait sur les onglets émoji.** Le panneau arme son balayage de fermeture sur tout ce qu'on presse dans ses soixante premiers pixels, et les onglets étaient mesurés à soixante et un : un pixel de marge, sur un téléphone. Elle s'arrête maintenant là où commencent les commandes, calculé plutôt que deviné.
- **« Developed by vyrriox » était écrit deux fois** dans les Réglages, une fois en rangée et une fois en phrase dessous. La rangée reste, c'est l'attribution que demande la licence ; la phrase est retirée.
- **`phone:Call(source, numero)` et `phone:EndCall(source)`.** Un script peut passer et terminer un appel. Cela passe PAR le composeur du téléphone plutôt qu'à côté, donc toutes les règles que rencontre un joueur s'appliquent : l'appelant a besoin d'un téléphone et de réseau, le destinataire doit être connecté et pas déjà en ligne, Ne pas déranger est respecté, et qui se tient près du téléphone qui sonne l'entend. `{ anonymous = true }` masque le numéro, `{ video = true }` en fait un appel vidéo. Terminer une conférence en retire cette personne plutôt que de la finir pour tout le monde.
- **`phone:GetMessages(citizenid, numero, limite)`** - ce qui a été dit dans une conversation, du plus ancien au plus récent. La lecture ne marque **pas** le fil comme lu : un script qui demande ce qui a été dit ne l'a pas lu à la place du joueur, et effacer sa pastille en silence serait un effet de bord que personne n'a demandé et que personne ne peut voir. Les messages que ce personnage a supprimés de sa propre copie restent masqués : ce n'est pas un moyen de lire au-delà du bouton supprimer.
- **Un compte épargne illisible dit maintenant pourquoi.** Trois situations sont identiques vues de l'application - pas de compte, une table absente, une lecture qui a échoué - et une seule est normale. La console dit laquelle, une fois, la première fois que ça arrive.
- **`tools/probe.js`** - la question qu'aucune vérification statique ne peut poser : le téléphone dessiné, un curseur pourrait-il vraiment cliquer ceci ? Hors écran, taille nulle, recouvert, ou seulement accessible en faisant défiler un conteneur latéralement comptent tous pour non. Il installe d'abord les trente-trois applications, parce que la préview démarre comme une installation neuve et que douze sont conditionnées : vingt et une sur trente-trois n'est pas un balayage. C'est la vérification qui aurait attrapé les onglets émoji, et elle a trouvé les catégories du magasin dès son premier passage.
...
- **A phone in a pocket never made a sound** ([#7](https://github.com/laforetbrut/v-phone-fivem/issues/7)). The only thing in the page that plays a notification is the banner, and the banner is only reached while the phone is OPEN. A closed phone was told to file the card and lift the handset, and both of those are silent - so the alert tone worked when you happened to be looking at the screen and never when the phone was away, which is not "closed phones are quiet", it is "the ringtone works sometimes". It sounds now, from the archive rather than the peek, so turning off the handset animation does not take the sound with it.
- **The ringtone restarted on every repaint.** The call screen asks for the ring whenever the call is inbound, and it is redrawn on every push the server makes about that call - the roster, the signal, the state, the timer. Asking for the ring tore it down and started it again, so a player's own MP3 never got past its first second and the built-in reset its interval before it could come round. The ring is asked for by description now and started only if that description changed: ten requests in a row play one ringtone.
- **A ringing phone could ring past the end of everything.** Every path that starts the ring stops it and every one has been checked, but "the ringtone will not stop" follows a player until they reconnect, so there is a check every two seconds as well: ringing with no call to ring for is over, whatever the reason, including one nobody has found yet. The game-side ring is also released when the resource stops - `Remote_Ring` loops until something stops it, and a restart was not something, which on a live server is the common case rather than the rare one.
- **A contact's name printed through the back button.** The small title is centred across the whole bar, which is right, and it was kept clear of "< Messages" by a fixed inset on one and a fixed width on the other. Two numbers chosen separately do not add up: measured in the real page, the back label ended at 110px and "James Thompson" began at 107. It measures now instead of guessing, and when a name genuinely cannot share the bar the back LABEL goes and the chevron stays - the chevron is the control, the word beside it is a courtesy.
- **A balance that had stopped being true.** The bank read it when the app opened and never again, so money taken by a shop or paid by a job left the old number on screen for as long as the player stayed there. Reported as "it takes ages to appear", which is what a screen that never refreshes looks like from outside. Both bank apps redraw when money moves - and the redraw sits ABOVE the notification settings on purpose, because announcing a movement and updating a screen are different questions: notifications for outgoing money are off by default, so a PURCHASE, the exact case reported, would have returned before a refresh placed with the notification code ever ran.

### Added

- **Leave a group.** There was no way out of one: a player added to a conversation they wanted no part of could delete the messages and watch the thread come back with the next one. The messages stay with the people still in it - a leaver is not entitled to take that with them - and a group nobody is left in is deleted.
- **A different sound for each app.** One tone for everything means a phone that tells you something happened and nothing about what. Money, a message, a dispatch, a social notification and a delivery are distinguishable from a pocket now without being looked at. An app with no sound of its own falls back to your alert tone, so a drop-in app is never silent, and a custom sound file still overrides all of it.
- **The savings account is read from doc-banking properly.** The first attempt guessed at export names; doc-banking publishes none for savings, and the balance reaches its own interface through an internal callback. Checked against the shipped resource and read from its table instead - a read, never a write, which is why the phone shows the figure and offers no way to move it.
- **The reason codes this server actually writes**, taken from the doc-* resources rather than guessed: the taxi's deposits and tips, the restaurant's orders and refunds, the lottery's tickets and wins, and second-hand vehicles.
- **`tools/check.py`** - the five failures that need no game and that nothing else would notice, because none of them is an error: a `post()` with no handler (the screen waits for ever), a locale key that does not exist (the key prints at the player), a class with no rule (an element drawn with no styling), a tone name that resolves nowhere (a notification answers with the call ringtone), and a query whose parameters do not match its placeholders (the wrong value bound into the wrong column). Every check carries a `--selftest` that proves it can still fail, because a checker nobody has watched fail is a checker nobody knows the shape of - and one that cries wolf is worse than none. It found a missing handler in this release's own new code before it shipped.
...
- **Un téléphone dans la poche ne faisait aucun bruit** ([#7](https://github.com/laforetbrut/v-phone-fivem/issues/7)). La seule chose qui joue une notification dans la page est la bannière, et la bannière n'est atteinte que téléphone OUVERT. À un téléphone fermé on demandait de classer la carte et de sortir le combiné, deux gestes muets : la tonalité marchait quand on regardait l'écran et jamais quand le téléphone était rangé, ce qui n'est pas « un téléphone fermé est silencieux » mais « la sonnerie marche parfois ». Elle sonne désormais, depuis l'archive et non depuis le lever du combiné, pour que désactiver l'animation n'emporte pas le son avec elle.
- **La sonnerie redémarrait à chaque redessin.** L'écran d'appel demande la sonnerie dès que l'appel est entrant, et il est redessiné à chaque information que le serveur envoie sur cet appel : la liste, le signal, l'état, le minuteur. Demander la sonnerie la détruisait et la relançait : un MP3 personnel ne dépassait jamais sa première seconde et l'intégrée remettait son intervalle à zéro avant qu'il ne revienne. La sonnerie est maintenant demandée par description et lancée seulement si cette description a changé : dix demandes de suite jouent une sonnerie.
- **Une sonnerie pouvait survivre à tout.** Chaque chemin qui la lance l'arrête et tous ont été vérifiés, mais « la sonnerie ne s'arrête plus » poursuit un joueur jusqu'à sa reconnexion : il y a donc aussi une vérification toutes les deux secondes, et sonner sans appel à annoncer est terminé quelle qu'en soit la raison, y compris une que personne n'a encore trouvée. Le son côté jeu est aussi libéré à l'arrêt de la ressource : `Remote_Ring` boucle jusqu'à ce qu'on l'arrête, et un redémarrage ne l'arrêtait pas, ce qui sur un serveur vivant est le cas courant et non le cas rare.
- **Le nom d'un contact s'imprimait par-dessus le bouton retour.** Le petit titre est centré sur toute la barre, ce qui est juste, et il était tenu à l'écart de « < Messages » par une marge fixe d'un côté et une largeur fixe de l'autre. Deux nombres choisis séparément ne s'additionnent pas : mesuré dans la vraie page, le libellé retour finissait à 110 px et « James Thompson » commençait à 107. Cela se mesure maintenant au lieu de se deviner, et quand un nom ne peut vraiment pas partager la barre, c'est le LIBELLÉ retour qui part et le chevron qui reste : le chevron est la commande, le mot à côté est une politesse.
- **Un solde qui avait cessé d'être vrai.** La banque le lisait à l'ouverture de l'application et plus jamais : l'argent pris par une boutique ou versé par un job laissait l'ancien chiffre à l'écran tant que le joueur y restait. Rapporté comme « ça met du temps à s'afficher », ce à quoi ressemble de l'extérieur un écran qui ne se rafraîchit pas. Les deux applications bancaires se redessinent quand l'argent bouge, et ce rafraîchissement est placé AU-DESSUS des réglages de notification volontairement : annoncer un mouvement et mettre un écran à jour sont deux questions différentes, les notifications d'argent sortant sont désactivées par défaut, donc un ACHAT - le cas exact rapporté - serait sorti de la fonction avant qu'un rafraîchissement placé avec le code de notification ne s'exécute.
- **Quitter un groupe.** Il n'y avait aucune sortie : un joueur ajouté à une conversation dont il ne voulait pas pouvait supprimer les messages et voir le fil revenir au message suivant. Les messages restent avec ceux qui y sont encore - qui part n'a pas le droit de les emporter - et un groupe que plus personne n'occupe est supprimé.
- **Un son différent par application.** Une seule tonalité pour tout, c'est un téléphone qui dit que quelque chose est arrivé et rien sur quoi. L'argent, un message, un dispatch, une notification sociale et une livraison se distinguent maintenant depuis la poche sans regarder. Une application sans son propre retombe sur votre tonalité d'alerte, donc une application tierce n'est jamais muette, et un fichier son personnalisé continue de tout supplanter.
- **Le compte épargne est lu correctement depuis doc-banking.** La première tentative devinait des noms d'export ; doc-banking n'en publie aucun pour l'épargne, et le solde n'atteint sa propre interface que par un callback interne. Vérifié sur la ressource livrée et lu depuis sa table : une lecture, jamais une écriture, ce qui explique que le téléphone affiche le montant sans offrir de le déplacer.
- **Les motifs que ce serveur écrit vraiment**, relevés dans les ressources doc-* plutôt que devinés : cautions et pourboires du taxi, commandes et remboursements du restaurant, tickets et gains de la loterie, et les véhicules d'occasion.
- **`tools/check.py`** - les cinq pannes qui ne demandent pas de jeu et que rien d'autre ne remarquerait, parce qu'aucune n'est une erreur : un `post()` sans gestionnaire (l'écran attend indéfiniment), une clé de locale inexistante (la clé s'affiche au joueur), une classe sans règle (un élément dessiné sans aucun style), un nom de tonalité qui ne résout nulle part (une notification répond avec la sonnerie d'appel), et une requête dont les paramètres ne correspondent pas à ses marqueurs (la mauvaise valeur dans la mauvaise colonne). Chaque vérification a un `--selftest` qui prouve qu'elle sait encore échouer, parce qu'un vérificateur que personne n'a vu échouer est un vérificateur dont personne ne connaît la forme, et qu'un vérificateur qui crie au loup est pire que pas de vérificateur. Il a trouvé un gestionnaire manquant dans le code neuf de cette version avant qu'il ne sorte.
